...
public class GetProductsByStatusController {
    ...
    public ResponseEntity<PaginatedResponse<ProductDto>> getProductByStatus(
        ...
        final PaginatedResponse<Product> paginatedResponse = getProductsByStatusInputGateway.getProductsByStatus(productStatus, paginationRequest);
        final List<ProductDto> foundDtos = paginatedResponse.content().stream()
                .map(productMapper::toDto)
                .toList();

        GetProductsByStatusController.configureHateaos(foundDtos, pageable, paginatedResponse, status);
    ...
    private static void configureHateaos(List<ProductDto> foundDtos, Pageable pageable, PaginatedResponse<Product> paginatedResponse, boolean status) {
        foundDtos.forEach(productDto -> {
            ...
            productDto.add(WebMvcLinkBuilder.linkTo(
                            WebMvcLinkBuilder.methodOn(GetProductsByStatusController.class)
                                    .getProductByStatus(status, pageable)
                    ).withSelfRel()
            );
        });
